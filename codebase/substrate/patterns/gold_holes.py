...
def _circular_feature_geometry(params: dict, pattern_model: str) -> dict:
    from .nanopillars import _resolve_nanopillar_parameters
    ...
    if pattern_model == "fiducial_dots":
        diameter_um = _dimension_um_from_keys(
            ...
            "dot_diameter_nm",
            _dimension_um(params, "pillar_diameter_um", 1.0),
        )
        if "fiducial_dot_pitch_um" in _pattern_dimensions(params) or "pitch_nm" in _pattern_dimensions(params):
            pitch_um = _dimension_um_from_keys(params, "fiducial_dot_pitch_um", "pitch_nm", diameter_um + 2.0)
        else:
            pitch_um = None
        spacing_um = float(
            _pattern_dimensions(params).get(
                "fiducial_dot_edge_to_edge_spacing_um",
                _pattern_dimensions(params).get("pillar_edge_to_edge_spacing_um", 2.0),
            )
        )
        ...
        return {
            ...
            "feature_is_material": True,
            "feature_factor": _dimension_factor(params, "fiducial_dot_intensity_factor", 1.5),
            "background_factor": _dimension_factor(params, "fiducial_background_intensity_factor", 1.0),
        }
    if pattern_model == "patterned_coverslip":
        diameter_um = _dimension_um_from_keys(params, "coverslip_patch_diameter_um", "coverslip_patch_diameter_nm", 5.0)
        if "coverslip_patch_pitch_um" in _pattern_dimensions(params) or "coverslip_patch_pitch_nm" in _pattern_dimensions(params):
            pitch_um = _dimension_um_from_keys(params, "coverslip_patch_pitch_um", "coverslip_patch_pitch_nm", diameter_um + 5.0)
        else:
            pitch_um = None
        spacing_um = float(_pattern_dimensions(params).get("coverslip_patch_edge_to_edge_spacing_um", 5.0))
        if not np.isfinite(spacing_um) or spacing_um < 0.0:
        ...
        return {
            ...
            "radius_um": 0.5 * diameter_um,
            "feature_is_material": True,
            "feature_factor": _dimension_factor(params, "coverslip_patch_intensity_factor", 1.08),
            "background_factor": _dimension_factor(params, "coverslip_background_intensity_factor", 1.0),
        }
