...
- `config.PARAMS`
- `config._KNOWN_INTERNAL_PARAM_KEYS`
...
1. Any key in `config._KNOWN_INTERNAL_PARAM_KEYS` is **Hidden**.
...
- **Hidden**: only internal/instrumentation-only keys in
  `config._KNOWN_INTERNAL_PARAM_KEYS` are kept internal.
...
The above counts are generated by `scripts/generate_param_exposure_matrix.py` and mirrored in `docs/param_exposure_summary.md`.
...
- `docs/param_exposure_matrix.json` (auto-generated by `scripts/generate_param_exposure_matrix.py`)
