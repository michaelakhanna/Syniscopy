...
Syniscopy computes a complex scattered field by Fourier-propagating a pupil function. In scalar mode this yields a scalar amplitude spread function (ASF); in vectorial Debye mode it yields polarization components $(E_x,E_y,E_z)$. With vacuum wavelength \(\lambda_0\), medium refractive index \(n_m\), medium wavenumber \(k_m=2\pi n_m/\lambda_0\), transverse wavevector $\mathbf{k}_\perp=(k_x,k_y)$, pupil amplitude $P(\mathbf{k}_\perp)$, and $k_z(\mathbf{k}_\perp)=\sqrt{k_m^2-|\mathbf{k}_\perp|^2}$ for propagating components,
\begin{equation}
  \mathrm{ASF}(x,y,z)=
  ...
  d^2\mathbf{k}_\perp .
\end{equation}
The normalization is absorbed into $P$ under the chosen Fourier-transform convention. Syniscopy evaluates the field by pupil propagation using the fast Fourier transform \cite{fft-pupil}. For scalar coherent paths, it applies the Lorenz--Mie $S_2(\theta)$ amplitude to the pupil-spreader as a forward-scattering approximation for finite spheres; vectorial Debye applies polarization-resolved pupil fields with $S_1$/$S_2$ terms when material refractive index and diameter are available. Particle geometry is not restricted to spheres: the per-component scattered-field amplitude is set by a geometry-aware scattering model selected by shape and validity regime --- Lorenz--Mie for spheres, a quasi-static orientation-dependent polarizability tensor for ellipsoid and cylinder primitives, and a Born/Rayleigh--Gans Fourier form factor for general or voxelized/imported geometry --- with Lorenz--Mie restricted to spheres so a non-spherical shape is never rendered through a sphere-only scattering law. For analytic-polarizability primitives, the vectorial path first forms the component-wise dipole response $\mathbf{p}=R\alpha_{\mathrm{body}}R^\top\mathbf{e}_{\mathrm{inc}}$ and then propagates that dipole through the vectorial Debye operator; full-vector detection retains the resulting $(E_x,E_y,E_z)$ field, analyzer modes project the transported field, and scalar or incoherent fallbacks use a declared magnitude reduction. Syniscopy caches the result as a stack of two-dimensional complex fields over axial position for each unique sub-particle type. The default optical backend is vectorial Debye (polarization-aware pupil and field-component handling); scalar paraxial remains available through the \texttt{scalar\_paraxial} selector. Syniscopy adopts the $e^{+i\omega t}$ time convention; complex field amplitudes are the complex conjugate of references using $e^{-i\omega t}$ (e.g., \texttt{miepython}~\cite{miepython-software}, \texttt{tmm}~\cite{tmm-byrnes}). All real-valued observables---intensity, contrast, scattering cross-sections, and Cram\'er--Rao bounds---are independent of this choice.
...
I_{\mathrm{DHM}} &= |E_{\mathrm{obj}}+E_{\mathrm{sca}}+aE_{\mathrm{obj}}e^{i\mathbf{K}\cdot\mathbf{r}}|^2, \\
I_{\mathrm{iSCAT}} &= |E_{\mathrm{ref}}+E_{\mathrm{sca}}|^2 .
\end{align}
Here dark-field microscopy, coherent bright-field imaging, quantitative phase imaging, reflection interference contrast microscopy, digital holographic microscopy, and interferometric scattering microscopy correspond to the displayed detector-domain observables. $E_{\mathrm{inc}}$ denotes the incident bright-field component, $E_{\mathrm{ref}}$ the explicit reference field, $\rho_s$ and $\rho_p$ substrate and particle-interface reflection coefficients, $\phi_{\mathrm{int}}$ the interface phase accumulated by the reflected/scattered components, and $\mathbf{K}$ the off-axis holography carrier wavevector. For off-axis holography, $E_{\mathrm{obj}}$ is the unscattered object/background field --- the off-axis object-arm instance of the same collinear unscattered field written $E_{\mathrm{inc}}$ for collinear bright-field --- and $a$ is the reference-arm amplitude ratio; the off-axis reference $aE_{\mathrm{obj}}e^{i\mathbf{K}\cdot\mathbf{r}}$ is a tilted, amplitude-scaled copy of the object background, so the simulator renders the raw single-shot interferogram --- the un-tilted object background is retained and an empty scene still records carrier fringes. The localization bound is then computed on a demodulated reconstructed-field contrast: the lateral Fisher path isolates the $+1$ Fourier sideband, removes the continuous detector carrier to baseband including any fractional FFT-bin residual, subtracts the deterministic empty-reference sideband, and uses the detector-grid complex object field to evaluate $\operatorname{conj}(\Delta S_{+1})/\operatorname{conj}(aE_{\mathrm{obj}})$. The conjugation is the algebraic consequence of the rendered $e^{+i\mathbf{K}\cdot\mathbf{r}}$ carrier; after this normalization the observable is the shift-covariant scattered-field contrast, equivalently the particle-dependent part of $E_{\mathrm{obj}}+E_{\mathrm{sca}}$ with fixed $E_{\mathrm{obj}}$. Raw-frame Poisson noise is propagated through the same sideband extraction, fractional-carrier correction, object-field normalization, and conjugation as a correlated real--imaginary covariance. The lateral derivative is therefore the same spectral gradient used by every other modality, evaluated on the reconstructed contrast rather than on the raw carrier frame or on the unnormalized sideband product.
...
Zernike phase contrast and differential phase contrast are represented as phase-ring and asymmetric-illumination families, respectively. Zernike phase contrast defaults to an objective-pupil phase-ring transfer normalized by numerical aperture and wavelength. Differential-phase contrast defaults to a vectorial Debye asymmetric-illumination model: opposite condenser half-disc source distributions are propagated through shifted objective pupils, their detector intensities are differenced and normalized, and \texttt{full\_vector} detection is used as the default vectorial observable. Finite source quadrature half-weights samples on the dividing axis so the two opposing half-disc measures do not double-count their common boundary. The DPC implementation also supports \texttt{analyzer\_x}, \texttt{analyzer\_y}, \texttt{incoherent\_sum}, and \texttt{unpolarized} vectorial detection modes. Split-pupil/Foucault detection and phase-gradient proxy options remain available only through explicit lower-fidelity backend settings, and scalar two-axis DPC is likewise an explicit lower-fidelity selector. Widefield fluorescence and total internal reflection fluorescence select between a parametric emission point-spread-function profile and the vectorial photophysics backend.
...
\end{equation}
Components are partitioned into electromagnetic-interaction clusters by their exact inter-component surface gap. Well-separated components are combined as a coherent sum of independently propagated fields, which is exact at separation; closely spaced components are instead solved as a coupled cluster --- through a quasi-static coupled-dipole model, a discrete-dipole (DDA) solve, a multi-sphere $T$-matrix for sphere clusters, or a spheroid $T$-matrix --- so near-field coupling, plasmon hybridization, and multiple scattering between adjacent components are modeled rather than omitted. The chosen interaction model, the cluster partition, and the model's validity regime are recorded in the scattering metadata, and a coupled solver reduces to the independent superposition in the large-gap limit. For the independently propagated (well-separated) case, pupil propagation is cached per sub-particle type, so each frame places $N$ transformed component fields rather than rerunning the propagation per component. The generated component-work record is therefore an operation-count diagnostic for cached field placement, not a wall-clock benchmark.
...
\begin{equation}
  ...
  \right],
\end{equation}
clipped to $[0,1]$. The factor of three is the implemented permissive radial envelope: one two-dimensional Brownian step has radial scale $\sqrt{4D_i\Delta t}$, and the support score decays after roughly three such radial scales rather than at the one-scale displacement. Let $a$ be the configured ambiguity-distance scale, let $\mathcal{C}$ be the set of competing particles, and write $\mathbf{x}_{i,t}^{xy}$ for the projected lateral image-plane coordinate of particle $i$. The frame-level ambiguity competition score is
\begin{equation}
  q_i =
  \sum_{j\in\mathcal{C}}
  \exp\!\left[-\frac{1}{2}
  \left(\frac{\lVert\mathbf{x}_{j,t}^{xy}-\mathbf{x}_{i,t}^{xy}\rVert}{a}\right)^2\right]
  +
  \sum_{j\in\mathcal{C}}
