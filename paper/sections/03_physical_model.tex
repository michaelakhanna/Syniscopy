...
Zernike phase contrast and differential phase contrast are represented as phase-ring and asymmetric-illumination families, respectively. Zernike phase contrast defaults to an objective-pupil phase-ring transfer normalized by numerical aperture and wavelength. Differential-phase contrast defaults to a vectorial Debye half-pupil asymmetric-illumination intensity transfer and supports \texttt{analyzer\_x}, \texttt{analyzer\_y}, \texttt{incoherent\_sum}, \texttt{unpolarized}, and \texttt{full\_vector} detection modes, with \texttt{full\_vector} as the default vectorial observable. The legacy phase-gradient and scalar two-axis options remain available only through explicit backend settings. Widefield fluorescence and total internal reflection fluorescence select between a parametric emission point-spread-function profile and the vectorial photophysics backend. Transmission electron microscopy selects among weak-phase CTF proxy \texttt{ctf\_proxy}, reduced-slice multislice proxy \texttt{multislice\_lite}, and Syniscopy split-step multislice \texttt{syniscopy\_multislice} modes. Scanning electron microscopy defaults to sliced-source Monte Carlo interaction-volume transport \texttt{monte\_carlo\_transport}; retained alternate routes include Syniscopy raster transport \texttt{syniscopy\_transport\_lite}, Gaussian-probe secondary-emission proxy \texttt{gaussian\_probe\_proxy}, interaction-volume proxy, and reference-kernel table \texttt{reference\_kernel\_table} modes. Each implemented modality profile exposes the same renderer hooks---illumination field, response function, sample-environment interaction, and noise model---and records the selected backend fidelity in profile-card metadata.
