...
\end{itemize}
The deterministic simulator-side checks and notebook diagnostics in Sections~\ref{sec:results} and \ref{sec:appendix-non-scalar-modalities} are intended for conceptual and software validation; they are not substitutes for physical-instrument calibration. The repository includes a separate validation layer under \path{validation/} that records the reference status of each check. Its fast gate combines runtime-contract checks, material single-source checks, closed-form detector/noise and Fisher checks, Brownian and rotational-diffusion checks, scalar and vectorial point-spread-function limits, modality-equation identities, and TEM/SEM diagnostic anchors. External-reference checks use independent packages only where that strengthens a claim: Mie amplitudes are cross-checked against \texttt{miepython} and classical Mie theory \cite{mie-scattering,miepython-software}, coherent thin-film amplitudes against the \texttt{tmm} transfer-matrix implementation \cite{tmm-byrnes}, and TEM multislice fixtures can be generated with abTEM and ASE for external comparison \cite{abtem-code,ase}. Checks that rely on analytic limits cite the underlying physical equation or reference model instead of being promoted to empirical instrument validation. A backend is therefore treated as reference-validated only when the selected external fixture or published-reference tolerance sweep has passed and the corresponding metadata hash/status is recorded.
...
The Segment Anything Model 2 example uses Syniscopy datasets as input to a downstream video-segmentation model. The training examples use lossless contrast-analysis PNG frame sequences and target masks; the raw-signal AVI is a camera-style audit preview, not the training target. Optional raw-camera frame sequences and raw signal/reference arrays serve as diagnostic records. The Segment Anything Model 2 workflow selects the requested target mask and propagates ignore masks and per-pixel loss weights so ignored pixels contribute zero loss.
